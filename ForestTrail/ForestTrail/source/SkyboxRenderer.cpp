...
namespace GE {
	...
	// This is because cubemap is sampled from cube vertices
	struct CubeVertex {
		float x, y, z;
		...
		CubeVertex(float x_, float y_, float z_) {
			x = x_;
			y = y_;
			z = z_;
		}
	...
	void SkyboxRenderer::createCubemap(std::vector<std::string> filenames) {
		...
		for (int faceNum = 0; faceNum < 6; faceNum++) {
			...
			// Get the format from the SDL object, it will be in SDL Image's format
			...
			// Release the surface object and memory associated with it as it is no longer needed
		...
		// Clamp the texture edge to deal with inaccuracies. If the xyz coords go over 1 or under 0, the sampler will wrap a texture resulting in artefacting at the egdes
	...
	void SkyboxRenderer::draw(Camera* cam) {
		...
		// Zero out translation as just interested in rotation to make sure skybox is oriented around the origin
		camView[3][0] = 0.0f;
		camView[3][1] = 0.0f;
		camView[3][2] = 0.0f;
		...
		// Define the structure of a vertex for OpenGL to select values from the vertex buffer and store in vertexLocation attribute
