	...
	bool Model::loadFromFile(const char* filename) {
		...
		for (int MeshIdx = 0; MeshIdx < pScene->mNumMeshes; MeshIdx++) {
			...
			for (int faceIdx = 0; faceIdx < mesh->mNumFaces; faceIdx++) {
				...
				// Extract a vertex from the mesh's main vertex array for each point in the face (3 as there are three vertices in a face - triangulated)
				for (int vertIdx = 0; vertIdx < 3; vertIdx++) {
					// Extract pos and tex coord based on its index number not directly from the mesh arrays
