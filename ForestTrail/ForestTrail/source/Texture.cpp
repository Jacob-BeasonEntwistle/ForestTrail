...
namespace GE {
	void Texture::loadTexture(std::string filename) {
		...
		// Get the format from the SDL object, it will be in the SDL Image's format
		format = surfaceImage->format->format;

		// Determine which OpenGL format based off the SDL format
		switch (format) {
		...
		// Select created texture for subsequent texture operations to set up the texture for OpenGL
		...
		// Configure how the texture will be manipulated when it needs to be reduced or increased when rendering onto an object.
